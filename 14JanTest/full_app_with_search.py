...
def extract_products_from_retailer_page(
    url: str, lang: str = "en", max_products: int = 5
) -> List[Dict]:
    """
    Fetch a retailer search results page and extract individual product listings.
    This handles cases where SearXNG returns a search page URL instead of product URLs.
    """
    try:
        from bs4 import BeautifulSoup
    except ImportError:
    ...
    try:
        ...
        domain = _normalize_domain(urlparse(r.url).netloc)

        # Define extraction patterns for different retailers
        extraction_configs = {
            "trendyol.com": {
                "product_container": "div.p-card-wrppr",
                "product_link": "a",
                "product_title": "span.prdct-desc-cntnr-name",
                "product_price": "div.prc-box-dscntd",
                "product_image": "img.p-card-img",
                "price_fallback": "div.prc-box-sllng",
            },
            "hepsiburada.com": {
                "product_container": "li[class*='productListContent']",
                "product_link": "a[class*='moria-ProductCard']",
                "product_title": "h3[data-test-id='product-card-name']",
                "product_price": "div[data-test-id='price-current-price']",
                "product_image": "img[class*='product-image']",
            },
            "n11.com": {
                "product_container": "li.columnContent",
                "product_link": "a.plink",
                "product_title": "h3.productName",
                "product_price": "span.newPrice",
                "product_image": "img.lazy",
            },
            "vivense.com": {
                "product_container": "div.product-item",
                "product_link": "a.product-link",
                "product_title": "div.product-name",
                "product_price": "span.price",
                "product_image": "img.product-image",
            },
            "ikea.com.tr": {
                "product_container": "div.plp-fragment-wrapper",
                "product_link": "a.plp-product-link",
                "product_title": "span.plp-product-title",
                "product_price": "span.plp-price__sr-text",
                "product_image": "img.plp-product-image",
            },
            "evidea.com": {
                "product_container": "div.product-card",
                "product_link": "a.product-link",
                "product_title": "div.product-title",
            ...
            "wayfair.com": {
                "product_container": "div[data-hb-id='ProductCard']",
                "product_link": "a[data-hb-id='ProductCardLink']",
                "product_title": "p[data-hb-id='ProductCardName']",
                "product_price": "span[data-hb-id='PriceDisplay']",
                "product_image": "img[data-hb-id='ProductCardImage']",
            },
            "ikea.com": {
                "product_container": "div.plp-fragment-wrapper",
                "product_link": "a.plp-product-link",
        ...
        config = None
        for retailer_domain, cfg in extraction_configs.items():
            if retailer_domain in domain or domain in retailer_domain:
                config = cfg
                break
        ...
        if config.get("use_generic"):
            # Generic extraction using common patterns
            results = _extract_products_generic(soup, base_url, domain, max_products)
        else:
            # Retailer-specific extraction
            containers = soup.select(config["product_container"])[: max_products * 2]
            ...
            for container in containers:
                ...
                try:
                    # Extract link
                    link_elem = container.select_one(config.get("product_link", "a"))
                    if not link_elem:
                    ...
                    # Skip if it's a search/category URL
                    if any(
                        x in product_url.lower()
                        for x in [
                            "/sr?",
                            "/ara?",
                            "/search?",
                    ...
                    # Extract title
                    title = ""
                    if config.get("product_title"):
                        title_elem = container.select_one(config["product_title"])
                        if title_elem:
                    ...
                    if not title:
                        ...
                        title_elem = container.find(
                            ...
                            and any(
                                t in str(x).lower()
                                for t in ["title", "name", "desc", "product"]
                            ),
                        )
                        if title_elem:
                            title = title_elem.get_text(strip=True)
                    ...
                    if not title:
                        title = (
                            link_elem.get("title", "")
                            or link_elem.get_text(strip=True)[:100]
                        )
                    ...
                    if config.get("product_price"):
                        price_elem = container.select_one(config["product_price"])
                        if not price_elem and config.get("price_fallback"):
                            price_elem = container.select_one(config["price_fallback"])
                        if price_elem:
                    ...
                    # Extract image
                    image_url = None
                    if config.get("product_image"):
                        img_elem = container.select_one(config["product_image"])
                        if img_elem:
                            image_url = (
                                img_elem.get("src")
                    ...
                    results.append(
                        {
                            ...
                            "image": image_url,
                            "snippet": "",
                            "extracted_from_search_page": True,
                        }
                    )
...
def _extract_products_generic(
    soup, base_url: str, domain: str, max_products: int
) -> List[Dict]:
    """
    Generic product extraction for unknown retailers.
    Uses common patterns to find product cards.
    """
    results = []
    ...
    for selector in container_selectors:
        ...
        if len(containers) >= 3:  # Found enough items
            break

    if not containers:
        # Try finding any links that look like product pages
        ...
        for link in all_links:
            ...
            if any(
                p in href.lower()
                for p in ["/p/", "/product/", "/urun/", "-p-", "/pd/", "/item/"]
            ):
                if href not in [p.get("url") for p in product_links]:
    ...
    for container in containers[: max_products * 2]:
        if len(results) >= max_products:
            break
        ...
        try:
            ...
            href = link.get("href", "")
            if not href or any(
                x in href.lower() for x in ["/search", "/ara", "?q=", "/category"]
            ):
                continue
            ...
            # Get title
            title = ""
            for tag in ["h2", "h3", "h4", "span", "div", "p"]:
                title_elem = container.find(
                    tag,
                    class_=lambda x: x
                    and any(t in str(x).lower() for t in ["title", "name", "desc"]),
                )
                if title_elem:
                    title = title_elem.get_text(strip=True)
                    break
            ...
            if price_elem:
                price = _extract_price_from_text(price_elem.get_text())
            ...
            results.append(
                {
                    ...
                    "domain": domain,
                    "price": price,
                    "currency": "TRY" if ".tr" in domain else "USD",
                    "image": image_url,
                    "snippet": "",
                    "extracted_from_search_page": True,
                }
...
def _extract_price_from_text(text: str) -> Optional[str]:
    ...
    patterns = [
        r"([\d.,]+)\s*(?:TL|₺)",  # Turkish Lira
        r"(?:TL|₺)\s*([\d.,]+)",  # Turkish Lira prefix
        r"\$([\d.,]+)",  # USD
        r"([\d.,]+)\s*\$",  # USD suffix
        r"€([\d.,]+)",  # Euro
        r"([\d.,]+)\s*€",  # Euro suffix
    ...
    for pattern in patterns:
        ...
        if match:
            ...
            try:
                val = float(price_str)
                if 1 < val < 1000000:  # Reasonable price range
                    return str(val)
            except:
                continue
...
def search_with_enrichment(
    ...
    Language-aware version with search page product extraction.
    """
    ...
    # Separate search pages from product pages
    search_page_results = []
    ...
    for result in results:
        ...
        is_search_page = any(ind in url for ind in search_page_indicators)

        if is_search_page:
            search_page_results.append(result)
        else:
            product_page_results.append(result)

    # Extract products from search pages
    extracted_products = []
    for search_result in search_page_results[:3]:  # Limit to top 3 search pages
        search_url = search_result.get("url")
        if search_url:
            extracted = extract_products_from_retailer_page(
                url=search_url,
                lang=lang,
                max_products=4,  # Get top 4-5 from each search page
            )
            extracted_products.extend(extracted)

    # Combine: product pages first, then extracted products
    all_results = product_page_results + extracted_products
    ...
    def product_url_score(result: Dict) -> int:
        url = (result.get("url") or "").lower()
        score = 0

        # Bonus for extracted products (they're confirmed product pages)
        if result.get("extracted_from_search_page"):
            score += 5

        # Bonus for having price/image
        if result.get("price"):
            score += 3
        if result.get("image"):
            score += 2
    ...
    unique_results.sort(key=lambda x: product_url_score(x), reverse=True)
    unique_results = unique_results[:max_results]

    # Enrich top results that don't already have data
    for i, result in enumerate(unique_results[:enrich_top]):
        # Skip if already has good data from extraction
        if (
            result.get("extracted_from_search_page")
            and result.get("price")
        ...
        if url:
            try:
                ...
                if enriched.get("image") and not result.get("image"):
                    result["image"] = enriched["image"]
                if enriched.get("price") and not result.get("price"):
...
def generate_fallback_results(query: str, lang: str = "en") -> List[Dict]:
    ...
    Now also attempts to extract products from these pages.
    """
    encoded_query = quote_plus(query)
    results = []

    if lang == "tr":
        # Turkish retailer fallbacks with direct extraction
        tr_fallbacks = [
        ...
        # Try to extract products from top 2 retailers
        for domain, search_url in tr_fallbacks[:2]:
            extracted = extract_products_from_retailer_page(
                url=search_url, lang=lang, max_products=3
            )
        ...
        # If extraction didn't work, add fallback links
        if len(results) < 4:
            for domain, url in tr_fallbacks:
        ...
        # English retailer fallbacks
        en_fallbacks = [
            ("amazon.com", f"https://www.amazon.com/s?k={encoded_query}&i=garden"),
            (
                "wayfair.com",
                f"https://www.wayfair.com/keyword.html?keyword={encoded_query}",
            ),
        ...
        # Try extraction for English retailers too
        for domain, search_url in en_fallbacks[:2]:
            extracted = extract_products_from_retailer_page(
                url=search_url, lang=lang, max_products=3
            )
            results.extend(extracted)
        ...
        if len(results) < 4:
            for domain, url in en_fallbacks:
                if not any(r.get("domain") == domain for r in results):
                    results.append(
                        {
