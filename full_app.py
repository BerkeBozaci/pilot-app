...
def build_search_queries(item: Dict, sites: List[str] = None) -> List[str]:
    ...
    for keyword, token in material_keywords:
        if keyword in m_low:
            ...
            break  # Only use first match to keep query focused

    mat_str = material_tokens[0] if material_tokens else ""

    # Build queries from most specific to least specific
    queries = []

    # Most specific: name + color + material
    if color and mat_str:
        queries.append(f"{color} {mat_str} {name}")
    ...
    # Name + material
    if mat_str:
        queries.append(f"{mat_str} {name}")

    # Style + name
    if style and "modern" in style.lower():
        queries.append(f"modern {name}")

    # Just the item name with "furniture" or "buy"
    ...
    return out[:5]  # Limit to 5 queries
...
def searx_search(
    ...
    if restrict_to_retailers and allowed_domains:
        # Strategy 1: Site-restricted query (may not work with all SearXNG instances)
        sites_str = " OR ".join(
            [f"site:{d}" for d in allowed_domains[:10]]
        )  # Limit sites
        query_variants.append(f"({sites_str}) {query}")

        # Strategy 2: Just add "buy" or "shop" to guide results
        query_variants.append(f"buy {query}")
        query_variants.append(f"shop {query} furniture")

    # Strategy 3: Plain query as fallback
...
def replace_furniture_with_mask_vertex_imagen(
    ...
    """
    Vertex AI Imagen masked edit using google-genai SDK.

    IMPORTANT:
    - Edit Image is supported in Vertex AI using imagen-3.0-capability-001.
    - We pass a RAW reference image + an explicit MASK reference image.
    - Returns {"success": True, "image_base64": "<jpeg b64>"} on success.
    """
    if not VERTEX_GENAI_AVAILABLE:
        return {
            "success": False,
...
"""
    ...
    if prod_bytes:
        prod_img = _genai_image_from_bytes(prod_bytes, "image/jpeg")
        reference_images.append(
            RawReferenceImage(reference_id=3, reference_image=prod_img)
        )

    # Call Imagen edit model
    try:
    ...
    try:
        ...
        if not imgs:
            return {"success": False, "error": "Vertex returned no generated_images."}

        out_img = imgs[0].image

        img_bytes = None
        if hasattr(out_img, "image_bytes") and out_img.image_bytes:
            img_bytes = bytes(out_img.image_bytes)
        elif hasattr(out_img, "data") and isinstance(out_img.data, (bytes, bytearray)):
            img_bytes = bytes(out_img.data)

        if not img_bytes:
            return {
                "success": False,
                "error": "Could not extract image bytes from Vertex response.",
            }

        # Some versions store bytes in dict-like structures
        if img_bytes is None and isinstance(out_img, dict) and "data" in out_img:
            val = out_img["data"]
            if isinstance(val, (bytes, bytearray)) and len(val) > 1000:
                img_bytes = bytes(val)

        if img_bytes is None:
            # Last resort: try show() path doesn't help programmatically; fail with debug
            return {
                "success": False,
                "error": "Could not extract image bytes from Vertex response.",
            }
        ...
        return {"success": False, "error": f"Vertex response parse failed: {e}"}
...
def apply_overrides_sequentially(
    ...
    for idx, (item_id, item) in enumerate(enabled_overrides):
        ...
        if use_vertex:
            ...
            result = replace_furniture_with_gemini_inpaint(
                base_image_b64=current_b64,
                mask_b64=mask_b64,
                item_name=item.get("name", "furniture"),
                replacement_description=replacement_desc,
                product_image_b64=product_img,
            )
...
def render_products_phase():
    ...
    for item in furniture_items:
        ...
        with st.expander(
            ...
            retailer_links = generate_retailer_links(item)
