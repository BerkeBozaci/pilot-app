...
def generate_design_with_gemini(
    ...
    Model: gemini-2.5-flash-image (stable, production ready)
    Alternative: gemini-2.5-flash-image-preview-05-20 (if stable not available)
...
Generate the redesigned room image."""
    ...
    # Try multiple model variants (Google sometimes changes model names)
    model_variants = [
        "gemini-2.5-flash-image",  # Stable production model
        "gemini-2.5-flash-image-preview",  # Preview model
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if "candidates" in data and len(data["candidates"]) > 0:
                        ...
                        # Check for text response (might indicate the model can't generate images)
                    ...
                    return {"error": last_error}  # Stop entirely
