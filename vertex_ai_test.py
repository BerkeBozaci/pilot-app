...
def searx_search(
    ...
    effective_query = (
        build_site_restricted_query(query, allowed_domains)
    ...
    for item in data.get("results", [])[: max_results * 3]:
        ...
        # Keep your existing blocklist behavior
        if any(b in domain for b in BLOCKLIST):
            continue

        # Hard enforce allowlist (defense-in-depth)
...
def apply_overrides_sequentially(
    ...
    for item_id, item in editable:
        ...
        if mode == "product":
            prod = selected_products.get(item_id, {})
            product_b64 = prod.get("image_b64")  # <-- this is what you already store

        if use_vertex:
            out = replace_furniture_with_mask_vertex_imagen(
                base_image_b64_jpeg=current_b64,
                mask_png_b64=mask_b64,
                replace_prompt=replace_prompt,
                item_name=item.get("name", "furniture"),
                product_image_b64_jpeg=product_b64,  # <-- PART 4 FIX
            )
        else:
...
def render_products_phase():
    ...
    with st.expander("⚙️ Search settings", expanded=True):
        ...
        restrict_to_retailers = st.checkbox(
            "Restrict search to RETAILERS only (site-limited)",
            value=False,
        )

        allowed_domains = get_allowed_retailer_domains()
        st.caption(
            f"Allowed retailer domains ({len(allowed_domains)}): "
            + ", ".join(allowed_domains[:20])
            + (" ..." if len(allowed_domains) > 20 else "")
        )
        ...
        if st.button("🔍 Search All Items", type="primary"):
            ...
            for idx, it in enumerate(furniture_items, start=1):
                ...
                product_matches[str(it.get("id"))] = {
                    "queries": queries,
                    "results": results,
                    "restricted": bool(restrict_to_retailers),
                    "allowed_domains": (
                        allowed_domains if restrict_to_retailers else None
                    ),
                    "no_match_reason": (
                        "No match from allowed retailers"
                        if restrict_to_retailers and not results
                        else None
                    ),
                }
        ...
        with st.expander(
            ...
            if not results:
                if data.get("restricted"):
                    st.warning(
                        "No match from the allowed RETAILERS sites for this item."
                    )
                else:
                ...
                with card:
                    ...
                    with c2:
                        ...
                        if st.button(
                            ...
                            img_b64 = download_image_to_base64(img) if img else None
